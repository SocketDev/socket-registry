/**
 * @file The fleet markdown-filename classification, shared by the edit-time
 *   `markdown-filename-guard` hook and the commit-time belt check
 *   `scripts/fleet/check/markdown-filenames-are-canonical.mts` so the two never
 *   diverge (1 classifier, 1 reference). Lives under `_shared/` (ships to
 *   members, survives the bundle-only cutover) because the check runs in
 *   members. The convention: README/LICENSE anywhere; a narrow SCREAMING_CASE
 *   allowlist at root / docs/ / .claude/ / package roots; everything else
 *   lowercase-with-hyphens under docs/ or .claude/.
 */
...
export function classifyMarkdownPath(absPath: string): Verdict {
  ...
  // A markdown file inside a vendored-source payload tree mirrors an EXTERNAL
  // project's layout (node-smol's additions/source-patched/doc/api, an
  // upstream/ reference tree): its name and location are upstream-dictated
  // source-tree content, not fleet docs.
  const payloadNorm = normalizePath(absPath)
  if (/\/(?:additions\/source-patched|upstream)\//.test(payloadNorm)) {
    return { ok: true }
  }
