...
export function findWheelhouseClaudeMd(cwd: string): string | undefined {
  ...
  // Walk up from cwd: try ../<name>/template/base/CLAUDE.md at each parent.
  // (The canonical seed moved from template/ to template/base/; the old
  // template/CLAUDE.md probe never matched, so this always returned undefined
  // and the local-edit guidance was never surfaced.)
  let dir = cwd
  for (let i = 0; i < 4; i += 1) {
...
const REPEAT_FINDING_PATTERNS: ReadonlyArray<{ label: string; regex: RegExp }> =
  [
    {
      label: 'again',
      // "again", "once more", or "hit this again" — top-level alternation with an optional group
    ...
    },
    {
      ...
      regex:
        // Up to ~40 chars between "same" and "as/we saw" to match multi-word X without swallowing entire sentences
        /\bsame\s+[^.?!\n]{1,40}?\s+(?:as|we saw)\s+(?:before|earlier|last time|previously)\b/i,
    },
    {
      label: "we've seen this before",
      regex:
        // "we've/I've/we have/I have seen this already/before" — two alternation groups
        /\b(?:i have|i'?ve|we have|we'?ve)\s+seen\s+this\s+(?:already|before)\b/i,
    },
    {
      label: 'recurring / keeps happening',
      regex:
        /\b(?:keeps happening|kept happening|recurring|repeated|repeating)\b/i,
    },
  ]
