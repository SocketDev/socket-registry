...
// as gh-aw's maintenance workflow (`agentics-maintenance.yml`) carries without
// the `.lock.yml` suffix the path exemption catches. Its `uses:` comments are
// the generator's own (`# v0.79.6`, no date), so the canonical stamp cannot be
// asked of it. Mirrored in the sync-scaffolding check
// `workflow-uses-comment.mts`.
