...
- After modifying a reusable workflow (`ci.yml`, `provenance.yml`, or the
  `weekly-update.yml` delegator)
- After `gh aw compile` regenerates a gh-aw `.lock.yml` — the cascade re-pins the
  delegator's `uses:` to it; the `.lock.yml` itself is gh-aw-owned, not cascaded
  (`cascade-workflows.mts` skips it via `isGhAwGenerated`)
