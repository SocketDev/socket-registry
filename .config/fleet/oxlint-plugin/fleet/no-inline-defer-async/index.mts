...
function findInlineDeferOrAsync(text: string): Match | undefined {
  ...
  while ((m = SCRIPT_OPENER_RE.exec(text)) !== null) {
    ...
    return {
      opener: m[0],
      attr: (attrMatch.groups?.['attr'] ?? '').toLowerCase() as
        | 'defer'
        | 'async',
      offset: m.index,
    }
