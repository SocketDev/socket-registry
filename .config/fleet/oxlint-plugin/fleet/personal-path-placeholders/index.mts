...
const PATTERNS = [
  {
    // /Users/<user>/... — excludes a Windows drive-letter prefix (`C:/Users/...`),
    // which is the forward-slash Windows form the next pattern owns.
    re: /(?<![A-Za-z]:)(\/Users\/)<([^>]+)>(\/|$)/,
    canonical: 'user',
    label: '/Users/<user>/',
  },
...
const REAL_USERNAME_PATTERNS = [
  // Captures a macOS-style username: a letter followed by up to 31 alphanumeric/dash/underscore chars under `/Users/`.
  /(\/Users\/)([a-zA-Z][a-zA-Z0-9_-]{1,31})(\/)/,
  // Same shape for Linux: a real username segment under `/home/`.
      ...
      TemplateLiteral(node: AstNode) {
        if (node.expressions.length !== 0) {
          ...
          for (const q of node.quasis) {
            checkText(node, q.value.cooked, { isComment: false })
          }
          return
        }
