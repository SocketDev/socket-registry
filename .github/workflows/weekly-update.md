  ...
  workflow_call:
    inputs:
      ...
      checkout-submodules:
        ...
        default: 'false'
      fix-model:
        description: 'Claude model for the fix-test-failures escalation'
        required: false
        type: string
        default: 'sonnet'
      fix-timeout-minutes:
    ...
    steps:
      ...
      - name: Check for actionable updates
        ...
        run: |
          ...
          if [ -f .config/lockstep.json ]; then
            set +e
            pnpm run lockstep --json > /tmp/lockstep-report.json 2>&1
            LOCKSTEP_EXIT=$?
            set -e
            [ "$LOCKSTEP_EXIT" = "2" ] && HAS_UPDATES=true
          fi
          if [ -f .gitmodules ] && [ ! -f .config/lockstep.json ]; then
            while IFS= read -r SM; do
              (cd "$SM" && git fetch origin --tags --quiet 2>/dev/null || true)
              BEHIND=$(cd "$SM" && git rev-list --count HEAD..origin/HEAD 2>/dev/null || echo 0)
              if [ "$BEHIND" -gt 0 ]; then HAS_UPDATES=true; break; fi
...
safe-outputs:
  ...
  # workflow (one engine/model per workflow → fix is its own workflow).
  dispatch-workflow:
    workflows: [fix-test-failures]
    max: 1
---
...
4. **If tests fail:** do NOT open a PR. Dispatch the `fix-test-failures` workflow via
