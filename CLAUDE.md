...
🚨 **Package manager: `pnpm`.** NEVER `npx`/`pnpm dlx`/`yarn dlx`/`<pm> exec` — `node_modules/.bin/<tool>` or `pnpm run`. NEVER `--experimental-strip-types`; never pipe install/check/test/build to `tail`/`head`. `scripts/**`+`.claude/hooks/**` import via the repo's `-stable` alias. **Python: NEVER `pip`** (use `pypa-tool`; dev `pipx`). Reserved `scripts/` dirs: `scripts/{fleet,repo}/` only. **Run pnpm from the repo root** — never `cd <subpackage> && pnpm …`; target a workspace project with `pnpm --filter <pkg> …` (bypass `Allow repo-root bypass`; `.claude/hooks/fleet/operate-from-repo-root-guard/`). Bypasses + hooks: [`tooling`](docs/claude.md/fleet/tooling.md).
...
🚨 **Package-manager auto-update OFF** (`brew`/`choco`/`winget`/`scoop`/`npm`/`pnpm`) so an invocation can't change a tool version or pull an unsoaked package mid-task; set by `setup-security-tools`, enforced at invocation (`.claude/hooks/fleet/package-manager-auto-update-guard/`). **CDN allowlist**: `curl`/`wget`/`fetch` only to approved public registries/CDNs, never internal hosts (`.claude/hooks/fleet/cdn-allowlist-guard/`). Bypasses + detail: [`tooling`](docs/claude.md/fleet/tooling.md).
...
🚨 **Every `template/` edit → same-turn dogfood cascade** (`node scripts/repo/sync-scaffolding/cli.mts --target . --fix`): the wheelhouse's own `.claude/`/`.config/` is the LIVE copy, so an un-cascaded edit leaves it stale (`.claude/hooks/fleet/dogfood-cascade-reminder/`). **Sync is dumb-bit propagation.** `pnpm run sync --target . --fix`, commit `chore(wheelhouse): cascade template@<sha>`, push. Don't analyze each file or write rationale — the template is truth. If a cascade won't apply (lockfile reject, soak window, broken hook), bump the blocker or defer + report. **Token spend: match model + effort to the job** — mechanical work uses a cheap/fast model at low/medium effort; premium tiers for judgment. Guidance: [`token-spend`](docs/claude.md/fleet/token-spend.md) (`.claude/hooks/fleet/token-spend-guard/`; bypass `Allow model bypass` / `Allow effort bypass`). <!--advisory-->
...
🚨 Never emit a raw secret to tool output, commits, comments, or replies; when blocked, rewrite — don't bypass. Secret VALUE shapes (`AKIA…`/`ghp_…`/`sktsec_…`/JWT/PEM) blocked at edit + commit time; redact `token`/`jwt`/`api_key`/`secret`/`password`/`authorization` fields when citing responses. Tokens live in env vars (CI) or the OS keychain (dev) — never in `.env*`/`.envrc`/dotfiles; never read the keychain or clipboard/screen from Bash/hooks. Canonical env var `SOCKET_API_TOKEN` (keychain stores `SOCKET_API_KEY`). Setup/rotate: `node .claude/hooks/fleet/setup-security-tools/install.mts [--rotate]`. Hooks + bypasses: [`token-hygiene`](docs/claude.md/fleet/token-hygiene.md).
