...
**Layout:** `/registry/src/` (TS source) → `/registry/dist/` (esbuild CJS); `/scripts/` (dev/build `.mts`); `/test/`; `/packages/npm/` (overrides). Primary export: `getManifestData(ecosystem, packageName)` from `@socketsecurity/registry`. **Commands:** `pnpm run build` / `test` / `check` / `fix` / `type` (tsgo) / `update` / `make-npm-override` / `release-npm`. **Build:** esbuild → CJS, unminified for Node ESM interop; post-build transform rewrites wrappers to `module.exports = {...}`. Lint oxlint, format oxfmt. **Node compat:** ≥18.0.0. 🚨 Forbidden ES2023+: `toReversed/toSorted/toSpliced/with()` — use `slice().reverse()` / `slice().sort()`. **GHA SHA-pin cascade:** procedure in `.claude/skills/updating-workflows/` (`/update-workflows`, full ref at `reference.md`). 🚨 Never type/guess SHAs — `git rev-parse origin/main` AFTER merge. 🚨 Never `sed`/`awk`/`perl -i` workflow YAML — use Edit. **CI:** `SocketDev/socket-registry/.github/workflows/ci.yml@<SHA>` (full SHA), Node 22/24 cross-platform, scripts `lint-ci` / `test-ci` / `type-ci`. **Testing:** 🚨 Never `--` before test paths — runs ALL tests. Coverage mandatory, never decrease; `c8 ignore` needs a reason ending with `.`. **Vitest:** `.config/vitest.config.mts`, forks (`singleFork`, `maxForks: 1`, `isolate: true`), 60s timeouts. **Deps:** tsgo + `@types/node` + unified `typescript-eslint`. 🚨 Never the split `@typescript-eslint/*` packages; never swap tsgo for tsc.

Full layout / commands / build / testing / dependency policy + scratch-doc convention in [`docs/claude.md/repo/architecture.md`](docs/claude.md/repo/architecture.md).
