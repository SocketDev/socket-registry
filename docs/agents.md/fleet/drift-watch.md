...
3. **the fleet source repo's `template/` tree** is the canonical
   ...
   Diverging is drift.
4. **`node scripts/sync-scaffolding/cli.mts --all`** (in the fleet source repo)
...
- `scripts/sync-scaffolding/`: drift detection + auto-fix tooling (canonical in the fleet source repo).
