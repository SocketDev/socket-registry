...
- `readme-fleet-shape-guard` — PreToolUse Edit/Write(root `README.md`): blocks a resulting README missing/reordering the 5 canonical level-2 sections, mentioning the wheelhouse repo name outside a code fence, invoking a sibling-repo relative path, or missing a follow badge.
...
- `no-new-config-guard` — blocks creating a new standalone config-DATA file (`.json`/`.yaml`/`.toml`) under `.config/`; per-repo config flows through the one per-member fleet settings file + its schema as a section, not a new file; bypass `Allow new-config bypass`
