...
- `repo-map-refresh` — SessionStart, fail-open. Detached-spawns `make-repo-map --write --changed` to keep the gitignored `.repo-map/` skeleton cache warm (incremental, only when the cache already exists), so `read-orientation-nudge` can point at a ready-made skeleton instead of a whole-file read.
