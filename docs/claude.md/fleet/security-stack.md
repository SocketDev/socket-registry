...
Persistent env vars (`SOCKET_X_DISABLED=1` in shell rc) defeat the discipline the layered enforcement is meant to provide. The hook authors deliberately removed escape hatches in 2026-05 (`SOCKET_GH_HYGIENE_DISABLED` and `SOCKET_GH_TOKEN_AGE_DISABLED` were both deleted after the Nx Console breach made the "failsafe-deny on supply-chain credentials" stance non-negotiable).
