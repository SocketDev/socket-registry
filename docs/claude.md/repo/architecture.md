...
🚨 **Forbidden ES2023+ array methods**: `toReversed()`, `toSorted()`, `toSpliced()`, `with()`. Use `slice().reverse()` / `slice().sort()` instead. These methods are Node 20+ only; we ship to 18 consumers.
