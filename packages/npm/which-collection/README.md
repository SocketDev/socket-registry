...
>A tested zero dependency drop-in replacement of [`which-collection`](https://socket.dev/npm/package/which-collection) complete with TypeScript types.
...
Prefer to do it yourself? Add `@socketregistry/which-collection`
...
```sh
pnpm install @socketregistry/which-collection
```
