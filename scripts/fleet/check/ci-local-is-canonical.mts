 ...
 *   `.claude/skills/fleet/onboarding-fleet-member/SKILL.md`). The skill DESCRIBES
 *   the local-CI path; this check ENFORCES it so the prose can't drift from
 *   reality:
 *
 ...
 *     script) in a bad cascade would otherwise run a different / no local CI than
 *     the skill + the cascaded `agent-ci-local.test.mts` assume.
 *   - agent-ci Dockerfile identity — `.github/agent-ci.Dockerfile` is
 *     OPTIONAL_IDENTICAL (opt-in; byte-identical to the template WHEN present). A
 *     drifted copy would bake a different pnpm than CI uses. Only enforced when a
 *     template copy is reachable (the wheelhouse, or a checkout that vendored
 *     it); a downstream repo without the template skips that half.
 *
 *   Scope: the repo this runs in (check --all is per-repo). Exit codes: 0 — the
 *   ci:local script (and Dockerfile, if present) are canonical; 1 — drift.
 */
...
const CANONICAL_CI_LOCAL =
  'agent-ci run --all --quiet --pause-on-failure --github-token'
