 ...
 *   of the three is presumed un-optimized (it drags the whole upstream tree into
 *   every clone) and fails here. Determination of the safe sparse pattern set is
 *   the `optimizing-submodules` skill's job; this gate keeps the result from
 *   silently regressing. The `# full-checkout:` escape hatch is for the rare
 *   genuine case (a crate built from its whole source tree, an upstream with no
 *   separable subtree). It must name a reason, so the choice is reviewable
 *   rather than an omission. Exit: 0 — every block is optimized; 1 — one or more
 *   is neither sparse, shallow single-branch, nor annotated. Usage:
