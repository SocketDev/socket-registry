 ...
 *
 *   Usage: node scripts/fleet/check/thin-workflows-can-fetch-the-payload.mts [--quiet]
 */
...
export function main(): number {
  ...
  if (!isThinMember(REPO_ROOT)) {
    if (!quiet) {
      logger.success(
        '[thin-workflows-can-fetch-the-payload] not a thin member (no bundle.ref) — the payload fetch is a no-op here.',
      )
    }
    return 0
  }
  ...
  if (unwired.length) {
    logger.fail(
      '[thin-workflows-can-fetch-the-payload] a thin member has workflow step(s) that cannot authenticate the bundle fetch:',
    )
    logger.error(
  ...
  if (!quiet) {
    logger.success(
      `[thin-workflows-can-fetch-the-payload] all fleet install/checkout steps across ${workflows.length} workflow(s) can mint a payload token.`,
    )
  }
  return 0
}
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: `Usage: node scripts/fleet/check/thin-workflows-can-fetch-the-payload.mts`,
}
