...
import { collectPackageTrustDetails } from './check-trusted-details.mts'
import type { PackageInfo } from './check-trusted-details.mts'
...
export async function checkTrustedPackage(
  ...
  collectPackageTrustDetails(info, allowedMaintainers, issues, successes)
...
async function collectPackagesToCheck(): Promise<Set<string>> {
...
async function main(): Promise<void> {
  const packagesToCheck = await collectPackagesToCheck()
