...
async function main(): Promise<number> {
  ...
  await withSpinner({
    ...
    operation: async () => {
      ...
      await update(['scripts/repo/npm/update-npm-package-json.mts'])
      await update(['scripts/repo/npm/update-manifest.mts', '--force'])
    },
    spinner: getCachedDefaultSpinner(),
  })
...
const SCRIPT_META: ScriptMeta = {
  ...
  help: `Usage: node scripts/repo/npm/release-npm-packages.mts [options]
