 ...
 */
/* max-file-lines: vendored — mirrors upstream assert parallel test file; splitting drifts from the source map to upstream. */
...
describe(`${eco} > ${sockRegPkgName}`, { skip }, () => {
  ...
  describe('edge cases', () => {
    it('should handle NaN', () => {
      ...
      expect(() => assert.strictEqual(Number.NaN, Number.NaN)).toThrow(
        assert.AssertionError,
      )
    })
