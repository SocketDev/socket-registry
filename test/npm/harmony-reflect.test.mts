...
describe(`${eco} > ${sockRegPkgName}`, { skip }, () => {
  ...
  it('should correctly implement getOwnPropertyDescriptor', () => {
    expect(harmonyReflect.getOwnPropertyDescriptor({ x: 1 }, 'x').value).toBe(1)
  ...
  it('should correctly implement set', () => {
    // biome-ignore lint/suspicious/noImplicitAnyLet: Test variable assigned in setter.
    let out
    const target = Object.create(
      {
        z: 3,
  ...
  it('should correctly implement apply', () => {
    expect(harmonyReflect.apply((x: number) => x, undefined, [1])).toBe(1)
