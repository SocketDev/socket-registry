...
describe(`${eco} > ${sockRegPkgName}`, { skip }, () => {
  ...
  describe('extra tests', () => {
    ...
    it('splits with capturing groups', () => {
      ...
      expect(split('test', /t/)).toEqual(['', 'es', ''])
      expect(split('test', /(?<t>t)/)).toEqual(['', 't', 'es', 't', ''])

      expect(split('test', 'es')).toEqual(['t', 't'])
      expect(split('test', /es/)).toEqual(['t', 't'])
      expect(split('test', /(?<es>es)/)).toEqual(['t', 'es', 't'])

      const reTest = /(?<t1>t)(?<e>e)(?<s>s)(?<t2>t)/
      expect(split('test', reTest)).toEqual(['', 't', 'e', 's', 't', ''])
    })

    it('splits with complex HTML pattern', () => {
      const reHtml = /<(?<slash>\/)?(?<tag>[^<>]+)>/
    ...
    it('splits with repeated capture groups', () => {
      expect(split('tesst', /(?<s>s)*/)).toEqual([
        't',
        undefined,
        'e',
        's',
      ...
      expect(split('tesst', /(?<s>s*)/)).toEqual(['t', '', 'e', 'ss', 't'])
      const reGreedyAll = /(?<s>s*?)/
